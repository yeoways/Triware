...
tf.app.flags.DEFINE_enum("placer_type", "aware",
                         ["default", "colocation", "fusion", "aware"],
                         "Placer type.")
...
class AwarePlacementGraphGenerator():
    class Node(object):
        """
          Node class
    ...
    def __init__(self, tf_graph, metagraph, op_perf, step_stats):
        self._tf_graph = tf_graph
        self.op_perf = op_perf
        self.step_stats = step_stats
        self.metagraph = metagraph
        self.out_d = self.output_dict = defaultdict(dict)
        self.cost_d = self.cost_dict = defaultdict(dict)
        self.node_dict = dict()
        self.parent_map = defaultdict(dict)

    def _add_nodes(self, op_graph, ungrouped_mapping, group_to_group):
        for i, tf_op in enumerate(self._tf_graph.get_operations()):
            duration = self.cost_dict[tf_op.name]
            out_memory = self.output_dict[tf_op.name]
            ungrouped_mapping[str(tf_op.name)] = get_raw_colocation_group(tf_op)[0]
            if get_raw_colocation_group(tf_op)[0] not in group_to_group:
                group_to_group[get_raw_colocation_group(tf_op)[0]] = get_raw_colocation_group(tf_op)[0]
            op_graph.add_node(str(tf_op.name),
                              ...
                              out_size=sum(out_memory),
                              aggregated_nodes=[str(tf_op.name)],
                              contraction={},
                              output_memory=out_memory)

            _LOGGER.debug("New node added. %d[%s] colocation_group=%s",
                          i, tf_op.name, op_graph.nodes[tf_op.name]["colocation_group"])
        ...
        return ungrouped_mapping, group_to_group

    @staticmethod
    def _reset_out(op_graph):
        for node in op_graph.nodes.items():
            op1_output_memory = []
            for out_edge in op_graph.out_edges(node[0], data=True):
                op1_output_memory.append(out_edge[-1]['num_bytes'])
            node[-1]['output_memory'] = op1_output_memory
            node[-1]['out_size'] = sum(node[-1]['output_memory'])

    def _add_edges(self, op_graph):
        """Adds edges between operators to the graph."""
        i = 0
        for tf_op in self._tf_graph.get_operations():
            for child_name, slot in self.node_dict[tf_op.name].children.items():
                edge_data = {
                    "id": i,
                    "num_bytes": self.node_dict[tf_op.name].output_memory[slot]
                }

                if op_graph.has_edge(tf_op.name, child_name):
                    # merge edge data only if a new tensor is transferred.
                    prev_edge_data = op_graph[tf_op.name][child_name]
                    prev_edge_data['num_bytes'] += edge_data['num_bytes']
                else:
                    op_graph.add_edge(tf_op.name, child_name, **edge_data)
                    i += 1
        assert i == op_graph.number_of_edges()
    ...
    def get_op_mem(self, op_perf, step_stats):
        out_d = {}
        for op in op_perf:
    ...
    def get_attributes(self):
        ...
        # iterate through all the nodes of the graph，遍历图中所有的节点，然后去生成他们之间父节点和子节点之间的依赖关系
        for node in self.metagraph.graph_def.node:
    ...
    def run(self):
        ...
        group_to_group = {}
        op_graph = nx.DiGraph()
        cost_dict, _ = self.get_op_costs(self.step_stats)
        output_dict = self.get_op_mem(self.op_perf, self.step_stats)
        ...
        self._add_nodes(op_graph, ungrouped_mapping, group_to_group)
        self._add_edges(op_graph)
        node_list = list(op_graph.nodes.items())
        for op_name, op_data in node_list:
            if op_graph.in_degree(op_name) == 0 and op_graph.out_degree(op_name) == 0:
                op_graph.remove_node(op_name)
                # ungrouped_mapping[op_name] = node_list[0][1]['colocation_group'][0]
                group_to_group[ungrouped_mapping[op_name]] = ''
                ungrouped_mapping[op_name] = ''

        self._reset_out(op_graph)
        return op_graph, ungrouped_mapping, group_to_group
...
def create_aware_placement_graph(tf_graph, metagraph, op_graph, step_stats):
    generator = AwarePlacementGraphGenerator(tf_graph, metagraph, op_graph, step_stats)
    return generator.run()
...
class ColocationGraphPlacer(DefaultPlacer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Gg = colocation_op_graph
        pos = nx.spiral_layout(Gg)  # 螺旋状的图片
    ...
    def _create_colocation_graph(op_graph):
        ...
        for op_id in op_graph:
            ...
            if colocation_group in new_index:
                ...
                target_node["persistent_memory"] += op_node["persistent_memory"]
                ...
                new_graph.add_node(i,
                                   ...
                                   persistent_memory=op_node["persistent_memory"],
...
class AwarePlacer(Placer):
    def __init__(self, tf_graph, devices, method, comm_cost_fn,
                 only_important_ops, metagraph, op_perf, step_stats, memory_fraction=1.0,
                 log_device_placement=None,
                 cost_dict=None, grouper=None, adjustment_method=None,
                 adjustment_with_memory_limit=None, sct_threshold=None,
                 **kwargs):
        ...
        self._memory_fraction = memory_fraction
        self._log_device_placement = log_device_placement
        self.ungrouped_mapping = {}
        self.metagraph = metagraph
        self.op_perf = op_perf
        self.step_stats = step_stats

        self.op_graph, self.ungrouped_mapping, self.group_to_group = create_aware_placement_graph(self.tf_graph, self.metagraph, self.op_perf, self.step_stats)
        grouper = grouper_lib.get_grouper(self.grouper)
        self.op_graph = grouper(self.op_graph, self.ungrouped_mapping, self.group_to_group)
    ...
    def _generate_fused_op_graph(
        ...
        for ops in group_to_ops.values():
            ...
            while len(internal_edges) > 0:
                ...
                #                   colocation_group=get_raw_colocation_group(tf_op)[0],
                #                   device=str(tf_op.device),
                ...
                for in_edge in fused_op_graph.in_edges(op2, data=True):
                    ...
                    if from_op == op1:
                        continue
                    if AwarePlacer._add_fused_edge(
                            fused_op_graph, from_op, op1, edge_data):
                        # if the new edge is a candidate,
                        # check it by adding it to internal_edges
                        if from_op in ops:
    ...
    @staticmethod
    def _create_colocation_graph(op_graph, ungrouped_mapping):
        """Creates a new simulation graph by considering the colocation rule.
        ...
        for op_name in op_graph:
            ...
            ungrouped_mapping[op_name] = group_name
            if colocation_group in new_index:
                target_node = new_graph.nodes[new_index[colocation_group]]
                # Update the existing node
                # sum up the computation time by assuming that a device runs
                ...
                target_node["contraction"][op_name] = op_node
                target_node["output_memory"] += op_node["output_memory"]
                _LOGGER.debug("Node updated. %s", str(target_node))
            else:
                # memory出错修改
        ...
        for op1_name, op2_name in op_graph.edges:
            ...
            if new_graph.has_edge(group1_name, group2_name):
                ...
                new_graph.add_edge(group1_name, group2_name)
    ...
    def _find_cycle_and_save_figures(self, colocation_op_graph):
        ...
        try:
            ...
            if FLAGS.resolve_cycle:
                ...
                placer_utils.save_placement_graph(
                    self.op_graph.subgraph(op_names_in_cycle),
                    "cycle_op_graph.eps",
                    figsize=(20, 20),
                    font_size=7,
                    with_colocation_group=True)
    ...
    def get_group_to_group(self):
        return self.group_to_group

    def get_colocation_op_graph(self):
        return self.colocation_op_graph

    def get_fused_op_graph(self):
        return self.op_graph


def get_placer(tf_graph, devices, comm_cost_coeffs, metagraph=None, op_perf=None, step_stats=None, placement_method=None,
    ...
    elif placer_type == "aware":
        placer_cls = AwarePlacer
    ...
    return placer_cls(
        tf_graph, devices, placement_method, comm_cost_fn, only_important_ops, metagraph=metagraph, op_perf=op_perf, step_stats=step_stats,
