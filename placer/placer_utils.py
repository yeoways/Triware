...
def prune_non_important_ops_aware(op_graph, important_ops):
    """Prunes non important ops.
    ...
    for op_id, op_data in op_graph.nodes.items():
        op_name = op_data['name']
        if op_name not in important_ops:
            op_names_to_remove.append(op_name)
    _LOGGER.info('# ops to prune: %d', len(op_names_to_remove))

    op_graph = op_graph.copy()
    op_graph.remove_nodes_from(op_names_to_remove)
    assert nx.algorithms.components.is_weakly_connected(op_graph)
    ...
    # edge_id = 0
    # for from_op_name, to_op_name, edge_data in op_graph.edges(data=True):
    #     edge_data['id'] = edge_id
    #     from_op_name = op_graph.nodes[from_op_name]['name']
    #     to_op_name = op_graph.nodes[to_op_name]['name']
    #     important_op_graph.add_edge(
    #         important_op_index[from_op_name],
    #         important_op_index[to_op_name],
    #         **edge_data)
...
def prune_rate_and_update_ops_aware(op_graph):
    """Prunes non important ops.
    ...
    for op_id, op_data in op_graph.nodes.items():
        op_name = op_data['name']
        if 'learning_rate' in op_name or 'GradientDescent/update' == op_name:
            op_names_to_remove.append(op_name)
    _LOGGER.info('# ops to prune: %d', len(op_names_to_remove))
